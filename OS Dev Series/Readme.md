...
-`DB`->DefineByte(1 byte),`DW`-DefineWord(2bytes),`DD`-DefineDoubleWord(4bytes),`DQ`-(8bytes),`RESB`-ReserveBytes(1byte),`RESW`-ReserveWord(2bytes),`RESD`-ReserveDouble(4 bytes),`RESQ`-array of ten reals(8bytes),`RESY`-one YMM Register,`RESZ`-32 ZMM registers these all designed to use in .bss.`int 0x80` to call interrupt.How push instruction words??? By first decrementing esp by four followed by placing its operand into the contents of the 32-bit location at address esp (the stack pointer). Since the esp instruction is decremented every time the push instruction is performed, the stack grows from high addresses to lower addresses.
...
-For compiling the assembly entry point and c kernel along with using our linker file to create exe/bootable kernel.Takes care of building ISO

# Sector and Boot-sector

-sized division of bootable disk, 512 bytes in size
-bootsector or bootlock is region on bootable device contains machine code loaded to RAM by system's builtin firmware,512 bytes on floppy disk
...
-```sh Instruction Set``` collection of entities like Instructions,registers,addressing modes,memory architecture,interrupt,exception handling,external I/O.8086,80286,80386,80486,Pentium,Pentium I,II,III are all x86 family.
